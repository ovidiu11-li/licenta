...
<div class="flex min-h-screen">
    ...
    <aside class="w-64 bg-white border-r border-gray-200 flex flex-col py-8 px-4 min-h-screen">
        <nav class="flex-1">
            <ul class="space-y-2">
                <li>
                    <a href="{{ route('admin.teachers') }}" class="flex items-center px-3 py-2 rounded-lg bg-blue-100 text-blue-700 font-semibold">
    ...
    <main class="flex-1 max-w-6xl mx-auto mt-10 p-6">
        <div class="bg-white p-8 rounded-xl shadow">
            <h1 class="text-2xl font-bold mb-6">Administrare profesori</h1>
            <form method="POST" action="{{ route('admin.teachers.create') }}" class="mb-8 flex gap-4 items-end">
                @csrf
                <div>
                ...
                <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded">Creează profesor</button>
            </form>
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody>
                    ...
                    <tr class="hover:bg-gray-50">
                        <form method="POST" action="{{ route('admin.teachers.update', $teacher->id) }}" class="contents">
                            @csrf
                            @method('PUT')
                            <td class="px-4 py-2">
                                <input type="text" name="name" value="{{ $teacher->name }}" class="border rounded px-2 py-1 text-sm w-full" />
                            </td>
                            <td class="px-4 py-2">
                                <input type="email" name="email" value="{{ $teacher->email }}" class="border rounded px-2 py-1 text-sm w-full" />
                            </td>
                            <td class="px-4 py-2 flex gap-2">
                                <button type="submit" class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded text-xs">Salvează</button>
                        </form>
                        <form method="POST" action="{{ route('admin.teachers.delete', $teacher->id) }}" onsubmit="return confirm('Sigur vrei să ștergi acest profesor?');">
                            @csrf
                            @method('DELETE')
            ...
            @if($teachers->count() === 0)
                <div class="text-gray-400 text-center py-8">Niciun profesor găsit.</div>
            @endif
        </div>
    </main>
</div>
